...
杰理OTA升级APP是一款专为使用杰理芯片的设备设计的在线升级工具。它允许用户通过蓝牙对设备进行固件升级，以确保设备始终拥有最新的功能和安全修复。
...
```dart
  static Future<int> getConnectWay() async {
   ...
   使用示例:await BleMethod.getConnectWay();
```

### 1.7 设置当前使用的通讯方式 (Android)
...
```dart
  ...
  使用示例:await BleMethod.isUseGattOverEdr();
```

### 1.11 设置是否使用GattOverEdr(IOS)
...
```dart
  ...
  使用示例:await BleMethod.getGattServiceUuids();
```

### 1.13 设置Gatt Service uuids(IOS)
...
### 1.21 设置BLE MTU请求值(范围:23~509）(Android)
```dart
...
### 1.37 获取WiFi的IP地址(返回格式: "http://[ip]:[port]")
```dart
